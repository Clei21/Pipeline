...
def extract_metadata(text):
    ...
    r_match = re.search(r"Rev(?:ision)?[: ]*([A-Za-z0-9]+)", text, re.I)
    if r_match:
        rev = r_match.group(1)
...
def content_area(page):
    ...
    esquerda = w * 0.05
    direita = w * 0.93
    return fitz.Rect(esquerda, topo, direita, base)
...
def process_pdf(pdf_path, output_dir):
    ...
    try:
        ...
        pages = []
        total_paginas_processadas = len(doc) - 1

        for i in range(total_paginas_processadas):
            page = doc[i]

            extracted = extract_page(page, i + 1, meta)
            if extracted:
...
if __name__ == "__main__":
    pdf_path = (
        r"C:/Users/cleid/OneDrive/Documentos/BaseDados/Montagem/"
        r"9- PCE-ENPP-3-135.pdf"
    )
